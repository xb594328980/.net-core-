...
namespace Sansunt.Infra.Tools.Randoms
{
    /// <summary>
    /// 随机数生成器
    /// </summary>
    public class RandomBuilder : IRandomBuilder
    {
        ...
        {
            var list = Sansunt.Infra.Tools.Helpers.Enum.GetItems<TEnum>();
            int index = _random.Generate(0, list.Count);
            return Sansunt.Infra.Tools.Helpers.Enum.Parse<TEnum>(list[index].Value);
        }
    }
}
