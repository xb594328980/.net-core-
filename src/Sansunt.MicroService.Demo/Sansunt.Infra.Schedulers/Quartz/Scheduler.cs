...
using Sansunt.Infra.Tools.Reflections;
using Qz = Quartz;

namespace Sansunt.Infra.Schedulers.Quartz
{
    /// <summary>
    /// Quartz调度器
    /// </summary>
    public class Scheduler : Tools.Schedulers.IScheduler
    {
        /// <summary>
        /// Quartz调度器
        /// </summary>
        ...
        public async Task ScanJobsAsync(params Assembly[] assemblies)
        {
            ...
            var jobs = Reflection.GetInstancesByInterface<Tools.Schedulers.IJob>(assemblies);
            if (jobs == null)
                return;
        ...
        /// 添加作业
        /// </summary>
        public async Task AddJobAsync<TJob>() where TJob : Tools.Schedulers.IJob, new()
        {
        ...
        public async Task AddJobAsync(Tools.Schedulers.IJob job)
        {
            if (!(job is JobBase quartzJob))
                throw new InvalidOperationException("Quartz调度器必须从Sansunt.Infra.Schedulers.Quartz.JobBase派生");
